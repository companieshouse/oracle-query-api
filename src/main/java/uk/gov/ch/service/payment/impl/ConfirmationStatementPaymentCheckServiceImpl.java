...
@Service
public class ConfirmationStatementPaymentCheckServiceImpl implements
        ConfirmationStatementPaymentCheckService {
    ...
    public ConfirmationStatementPaymentJson isConfirmationStatementPaid(String companyNumber,
        ...
        Optional<ConfirmationStatementPayment> confirmationStatementPayment
                = confirmationStatementPaymentCheckRepository.findPaymentsForPeriod(companyNumber,
                madeUpToDate);
        ...
        if (confirmationStatementPayment.isPresent()) {
            ...
            if (paidByTransactionId != null) {
                LOGGER.info(String.format(
                        "Company Number: %s Confirmation statement payment id %s found for made up to date %s",
                        companyNumber, paidByTransactionId, madeUpToDate));
                confirmationStatementPaymentJson.setPaid(Boolean.TRUE);
            } else {
                LOGGER.info(String.format(
                        "Company Number: %s Confirmation statement payment query returned result but no payment transaction id found for made up to date %s",
                        companyNumber, madeUpToDate));
                confirmationStatementPaymentJson.setPaid(Boolean.FALSE);
            }
        } else {
            LOGGER.info(String.format(
                    "Company Number: %s Confirmation statement payment query returned no result for made up to date %s",
                    companyNumber, madeUpToDate));
            confirmationStatementPaymentJson.setPaid(Boolean.FALSE);
        }
