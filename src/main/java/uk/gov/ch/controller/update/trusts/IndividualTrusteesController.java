...
public class IndividualTrusteesController {
    ...
    public ResponseEntity<List<IndividualTrusteeData>> getIndividualTrustees(@PathVariable String trustId) {
        DataMap dataMap = new DataMap.Builder().build();
        dataMap.getLogMap().put("trust_id", trustId);
        LOGGER.infoContext(trustId,
                String.format("Calling service to retrieve individual trustee data for trust ID: %s", trustId),
                dataMap.getLogMap());

        try {
            List<IndividualTrusteeData> details = individualTrusteesService.getIndividualTrustees(trustId);
            LOGGER.infoContext(trustId,
            ...
            LOGGER.infoContext(trustId,
                    String.format("No individual trustees could be found for trust ID: %s", trustId),
                    dataMap.getLogMap());
