...
public class StatementOfCapitalRepository {
    ...
    static final String STATEMENT_OF_CAPITAL_SQL =
            "SELECT coalesce(ssc.non_standard_share_class,sct.share_class_type_desc) as classOfShares, ctp.currency_type_code as currency,"
    ...
    public List<StatementOfCapital> getStatementOfCapital(String incorporationNumber)
        ...
        try {
            List<StatementOfCapital> statementOfCapitalList = jdbcTemplate.query(
                    STATEMENT_OF_CAPITAL_SQL, getParam(incorporationNumber),
                    new BeanPropertyRowMapper<>(StatementOfCapital.class));
            LOGGER.info("Returned statement of capital list " + statementOfCapitalList.size()
                    + " for company number " + incorporationNumber);
            return statementOfCapitalList;
        } catch (EmptyResultDataAccessException e) {
            LOGGER.error(
                    "No results were found when getting statement of capital for company number "
                            + incorporationNumber, e);
