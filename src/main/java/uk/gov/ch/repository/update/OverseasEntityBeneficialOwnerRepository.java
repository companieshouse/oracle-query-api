...
public interface OverseasEntityBeneficialOwnerRepository extends
    ...
    @Query(value = "select distinct "
            ...
            + "left join USUAL_RESIDENTIAL_ADDRESS URA on URA.USUAL_RESIDENTIAL_ADDRESS_ID = ("
            + "    select max(URA2.USUAL_RESIDENTIAL_ADDRESS_ID) "
            + "    from USUAL_RESIDENTIAL_ADDRESS URA2 "
            + "    where URA2.USUAL_RESIDENTIAL_ADDRESS_ID = OD.USUAL_RESIDENTIAL_ADDRESS_ID "
            + ") "
            + "left join CORBOD_APPT_OTHER_ADDR_LINK CBAOA on CBA.CORPORATE_BODY_APPOINTMENT_ID = CBAOA.CORPORATE_BODY_APPOINTMENT_ID "
            + "left join ADDRESS AD on AD.ADDRESS_ID = ("
            + "    select max(A2.ADDRESS_ID) "
            + "    from CORBOD_APPT_OTHER_ADDR_LINK CBAOA2 "
            + "    join ADDRESS A2 on CBAOA2.ADDRESS_ID = A2.ADDRESS_ID "
            + "    where CBAOA2.CORPORATE_BODY_APPOINTMENT_ID = CBA.CORPORATE_BODY_APPOINTMENT_ID "
            + ") "
