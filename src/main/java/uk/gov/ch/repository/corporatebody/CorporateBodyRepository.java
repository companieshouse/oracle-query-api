...
public class CorporateBodyRepository {
    ...
    public Long getTradedStatus(String companyNumber) throws CorporateBodyNotFoundException {
        try {
            Long tradedStatus = jdbcTemplate.queryForObject(GET_TRADED_STATUS_SQL, Long.class, companyNumber);

            LOGGER.info("Returning traded status " + tradedStatus + " for company number " + companyNumber);

            return tradedStatus;
        } catch (EmptyResultDataAccessException e) {
            LOGGER.error("No results were found when getting traded status for company number " + companyNumber, e);
