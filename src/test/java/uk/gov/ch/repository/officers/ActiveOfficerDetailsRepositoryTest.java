...
class ActiveOfficerDetailsRepositoryTest {
    ...
    void getActiveOfficerDetailsTest() throws NoActiveOfficersFoundException {
        ...
        when(jdbcTemplate.query(eq(ActiveOfficerDetailsRepository.OFFICER_DETAILS_SQL), any(PreparedStatementSetter.class), any(BeanPropertyRowMapper.class))).thenReturn(expectedList);

        ActiveOfficerDetails result = activeOfficerDetailsRepository.getActiveOfficerDetails(COMPANY_NUMBER);
    ...
    void getActiveOfficerDetailsOfCompanyWithNoneTest() {
        ...
        Assertions.assertThrows(NoActiveOfficersFoundException.class, () -> {
            activeOfficerDetailsRepository.getActiveOfficerDetails(COMPANY_NUMBER);
        });
    }
}
