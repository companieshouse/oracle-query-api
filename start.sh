...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/oracle-query-api/env

    PORT="${ORACLE_QUERY_API_PORT}"
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec java ${JAVA_MEM_ARGS} -Djava.util.logging.config.file="${APP_DIR}/logging.properties" -jar -Dserver.port="${PORT}" "${APP_DIR}/oracle-query-api.jar"
